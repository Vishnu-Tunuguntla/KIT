...
"""
create_barcode_images_table = """
CREATE TABLE IF NOT EXISTS BarcodeImages (
    BarcodeImageID SERIAL PRIMARY KEY,
    ItemID INT,
    Path TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS NutritionalFacts (
    NutritionalFactID SERIAL PRIMARY KEY,
    ItemID INT,
    Details JSONB,
    FOREIGN KEY (ItemID) REFERENCES FoodItem (ItemID)
);
...
cur.execute(create_barcode_images_table)
